...
public:
	ArrayIndexNode(const Position * p, LocNode * locIn, ExpNode * idxIn)
	: LocNode(p), myLoc(locIn), myIdx(idxIn){ }
	void unparse(std::ostream& out, int indent) override;
	virtual bool nameAnalysis(SymbolTable * symTab) override;
private:
	LocNode * myLoc;
...
public:
	ArrayTypeNode(const Position * p, TypeNode * eltType, int len)
	: TypeNode(p), myEltType(eltType), myLength(len){}
	void unparse(std::ostream& out, int indent) override;
	virtual const DataType * getType() const override;
private:
	TypeNode * myEltType;
